...
class CourseContentController extends Controller
{
    public function createChapterModal(Request $request, string $id): string
    {
        ...
        return view(
            'admin.course.course-module.partials.course-chapter-modal',
    ...
    public function createLesson(Request $request): string
    {
        $courseId = $request->course_id;
        $chapterId = $request->chapter_id;
        return view('admin.course.course-module.partials.chapter-lesson-modal', compact('chapterId', 'courseId'))->render();
    }
    ...
    public function sortChapter(string $id) {
        ...
        return view('admin.course.course-module.partials.course-chapter-sort-modal', compact('chapters', 'id'))->render();
    }
    ...
    public function updateSortChapter(Request $request,  string $id){
        ...
        foreach ($orderIds as $key => $orderId) {
            $chapter = CourseChapter::where(['id' => $orderId, 'course_id' => $id])->first();
            $chapter->order = $key + 1;
            $chapter->save();
        }
